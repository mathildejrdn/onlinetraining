...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $entrepot = htmlspecialchars($_POST['entrepot']);
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $prep = $conn->prepare($sql_insert);
            $prep->bind_param("is", $id_to, $message_content);
            $prep->execute();
            ...
            if (mail($secretary_email, $subject, $body, $headers)) {
...
<div class="flex items-center justify-center min-h-screen">
    <form class="max-w-md bg-white p-6 rounded shadow-md w-full" method="post" action="chemin-vers-envoi-du-message.php">
        ...
        <div class="relative z-0 w-full mb-5 group">
            <input type="text" name="phone_number" id="phone_number" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-red-600 peer" placeholder=" " required />
