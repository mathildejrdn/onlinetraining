...
if(!empty($_POST))
{
    if(isset($_POST["ref"], $_POST["marque"], $_POST["type"], $_POST["nom"], $_POST["couleur"], $_POST["matiere"], $_POST["motif"], $_POST["taille"], $_POST["genre"], $_POST["stock"], $_POST["prix"], $_POST["description"]) && !empty($_POST["ref"]) && !empty($_POST["marque"]) && !empty($_POST["type"]) && !empty($_POST["nom"]) && !empty($_POST["couleur"]) && !empty($_POST["matiere"]) && !empty($_POST["motif"]) && !empty($_POST["taille"]) && !empty($_POST["genre"])&& !empty($_POST["stock"]) && !empty($_POST["prix"]) && !empty($_POST["description"]))
    ...
    if (isset($_FILES["image"]) && $_FILES["image"]["error"] === UPLOAD_ERR_OK) 
        {
            ...
            require_once("../connect.php");
             ...
             // Écrire la requête
             $sql = "INSERT INTO `products`(`reference_produit`, `marque`, `image`, `type_de_produits`, `nom`, `couleur`, `matiere`, `stock`, `motif`,`taille`,`genre`,`prix`,`description`) VALUES (:reference_produit, :marque, :image, :type_de_produits, :nom, :couleur, :matiere, :stock, :motif, :taille, :genre, :prix, :description)";
...
<body class="bg-gray-100"><div class="flex items-center justify-center min-h-screen">
    <form class="max-w-md bg-white p-6 rounded shadow-md w-full" method="post" enctype="multipart/form-data">
        ...
        <div class="relative z-0 w-full mb-5 group">
            <input type="text" name="ref" id="ref" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-red-600 peer" placeholder=" " required />
        ...
        <!-- Couleur -->
        <div class="relative z-0 w-full mb-5 group">
            <input type="text" name="couleur" id="couleur" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-red-600 peer" placeholder=" " required />
        ...
        <!-- Matière -->
        <div class="relative z-0 w-full mb-5 group">
            <input type="text" name="matiere" id="matiere" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-red-600 peer" placeholder=" " required />
        ...
        <div class="relative z-0 w-full mb-5 group">
            ...
            <label for="genre" class="peer-focus:font-medium absolute text-sm text-red-500 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 peer-focus:text-red-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peexlate-y-6">Genre</label>
        </div>
        
        <!-- Quantité en stock -->
        <div class="relative z-0 w-full mb-5 group">
            <input type="number" name="stock" id="stock" min="0" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-red-600 peer" placeholder=" " required />
        ...
        <!-- Prix HT -->
        <div class="relative z-0 w-full mb-5 group">
            <input type="number" step="0.01" name="prix" id="prix" min="0" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-red-600 peer" placeholder=" " required />
