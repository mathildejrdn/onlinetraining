    ...
    <div class="hidden w-full md:block md:w-auto" id="navbar-dropdown">
      <ul
        ...
        <li class="relative">
          ...
          <!-- Menu déroulant - Femmes -->
          <?php
          include("./connect.php");
          $sql = "SELECT * FROM categories WHERE categorie_genre = 'femme'";
          ...
          <div id="womenDropdown"
            ...
            <ul class="py-2 text-sm text-gray-700">
              <?php foreach ($categories as $category) { ?>
                <li><a href="client_side/products_grid.php?genre=homme&categorie=<?= $category["nom_categorie"]?>"
                  class="block px-4 py-2 hover:bg-gray-100"><?= $category["nom_categorie"] ?></a></li>
              <?php } ?>
            </ul>
          </div>
        </li>
        <li class="relative">
          ...
          <!-- Menu déroulant - Hommes -->
          <?php
          $sql = "SELECT * FROM categories WHERE categorie_genre = 'homme'";
          $query = $db->prepare($sql);
          $query->execute();
          ...
          <div id="menDropdown"
            ...
            <ul class="py-2 text-sm text-gray-700">
              <?php foreach ($cats as $cat) { ?>
                <li><a href="client_side/products_grid.php?genre=homme&categorie=<?= $cat["nom_categorie"] ?>"
                  class="block px-4 py-2 hover:bg-gray-100"><?= $cat["nom_categorie"] ?></a></li>
              <?php } ?>
            </ul>
          </div>
        </li>
        ...
        <?php elseif (isset($_SESSION["admin"])) : ?>
          <li>
            <a href="../back_office/accueil.php" class="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-red-700 md:p-0">Administration</a>
          </li>
        <?php endif; ?>
      </ul>
    </div>
